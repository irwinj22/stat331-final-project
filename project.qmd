...
the following questions below your code:
   ...
   - Does this model explain more variance in `G3` than teh simple models from 
...
**10.** In Q3, you found that students tended to perform better in Portuguese
than in math on average. But is this difference statistically significant?
Conduct a paried t-test on `G3_math` and `G3_lang` from `both_classes` to
formally test this. Answer the following below your code:
   - State your null and alternative hypotheses before running the test.
...
t.test(both_classes$G3_math,
       both_classes$G3_lang,
       paired = TRUE)
```
