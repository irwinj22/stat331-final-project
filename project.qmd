...
**2.** Using the shared attributes listed in the data description, join the 
`math` and `lang` datasets so that each row represents one student enrolled in 
both courses. Before joining, rename `G1`, `G2`, and `G3` in each dataset to
`G1_math`, `G2_math`, `G3_math` and `G1_lang`, `G2_lang`, `G3_lang`, so it is 
clear which grades belong to which subject. Assign this to a new dataset called 
`both_classes`.
...
#| code-summary: "Click here to see the solution"
both_classes <- math |>
  rename(G1_math = G1, G2_math = G2, G3_math = G3) |>
  inner_join(
    lang |> rename(G1_lang = G1, G2_lang = G2, G3_lang = G3),
...
```{r}
addr_traveltime <- group_and_summarize(both_classes, address, traveltime_math)
addr_traveltime |>
  kable()
```
...
plot <- addr_traveltime |>
  mutate(
    address = case_when(
      address == "R" ~ "Rural",
      .default = "Urban"),
    traveltime_math = case_when(
      traveltime_math == 1 ~ "< 15",
      traveltime_math == 2 ~ "15-30",
      traveltime_math == 3 ~ "30-60",
      .default = "> 60"),
    traveltime_math = fct_relevel(traveltime_math, "< 15", "15-30", "30-60", "> 60")) |>
  ggplot(aes(x = traveltime, y = count, fill = address)) + 
...
#| code-fold: true
#| code-summary: "Click here to see solution"
plot(G3 ~ goout, data = math, pch = 19)
math.fit = lm(G3 ~ goout, data = math)
abline(math.fit, col = 'blue')
...
#| code-fold: true
#| code-summary: "Click here to see solution"
plot(G3 ~ studytime, data = math, pch = 19)
math.fit = lm(G3 ~ studytime, data = math)
abline(math.fit, col = 'red')
