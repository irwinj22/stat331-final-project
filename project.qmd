...
**1.** Using the shared attributes listed in the data description, join the `math`
and `lang` datasets so that each row represents one student enrolled in both
courses. Assign this to a new dataset called `both_students`.**
...
**2.** Using your joined dataset, create a new variable `g3_diff` that represents
...
```{r}
# convert to factor for now 
# NOTE: remove this once it is done in data cleaning
math <- math |>
  mutate(traveltime = as.factor(traveltime))
```

**3.** Write a function that will, given a dataframe, group by a column and the count 
the number of each distinct value within another column. That is, the output
should be a dataframe with three columns: the group by variable, the summarize 
variables, and the number of distinct occurrences.

This function should take in four parameters: 

- dataframe
- column to group by
- column to summarize
- (optionally) a name for the column with count values
...
#| code-fold: true
#| code-summary: "Click here to see the solution"
huh <- function(df, group_col, summary_col, col_name = "new_col") {
  df |>
...
**4.**Apply this function to the math dataset to prove that it works. Group by address
and summarize traveltime.
       
```{r}
huh(math, address, traveltime)
```
